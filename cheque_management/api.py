...
def pe_before_submit(self, method):
	comp_def = frappe.db.get_value("Company", self.company, "use_company_defaults")
	if comp_def == 1:
...
def pe_on_submit(self, method):
	...
	if self.mode_of_payment == "Cheque" and self.payment_type == "Receive":
		if comp_def == 1:
			...
			notes_acc = self.paid_to
			rec_acc = self.paid_from
		...
		rc.business_unit = self.business_unit
		...
		rc.receivable_notes_account = notes_acc
		rc.receivable_account = rec_acc
		#rc.cheque_status = 1
	...
	if self.mode_of_payment == "Cheque" and self.payment_type == "Pay":
		if comp_def == 1:
			...
			notes_acc = self.paid_from
			rec_acc = self.paid_to
		...
		pc.business_unit = self.business_unit
		...
		pc.payable_notes_account = notes_acc
		pc.payable_account = rec_acc
		#pc.cheque_status = 1
