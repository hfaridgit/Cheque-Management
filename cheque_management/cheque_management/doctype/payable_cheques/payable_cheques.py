...
class PayableCheques(Document):
	...
	def on_update(self):
		notes_acc = self.payable_notes_account
		rec_acc = self.payable_account
		if not notes_acc:
			frappe.throw(_("Payable Notes Account not defined "))
		elif len(notes_acc) < 4:
				frappe.throw(_("Payable Notes Account not defined"))

		if not rec_acc:
			frappe.throw(_("Default Payable Account not defined"))
		elif len(rec_acc) < 4:
			frappe.throw(_("Default Payable Account not defined"))
