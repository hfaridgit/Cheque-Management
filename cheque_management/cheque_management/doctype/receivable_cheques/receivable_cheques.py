...
class ReceivableCheques(Document):
	...
	def on_update(self):
		notes_acc = self.receivable_notes_account
		rec_acc = self.receivable_account
		if self.cheques_under_collection_account:
			uc_acc = self.cheques_under_collection_account
		if not notes_acc:
			frappe.throw(_("Receivable Notes Account not defined"))
		elif len(notes_acc) < 4:
			frappe.throw(_("Receivable Notes Account not defined"))

		if self.cheque_status in ["Cheque Deposited", "Cheque Collected", "Cheque Returned"]:
			if not uc_acc:
				frappe.throw(_("Cheques Under Collection Account not defined"))
			elif len(uc_acc) < 4:
				frappe.throw(_("Cheques Under Collection Account not defined"))

		if not rec_acc:
			frappe.throw(_("Default Receivable Account not defined"))
		elif len(notes_acc) < 4:
			frappe.throw(_("Default Receivable Account not defined"))
		...
		if self.cheque_status == "Cheque Rejected":
			...
			self.make_journal_entry(pe.paid_from, self.reject_account, self.amount, self.posting_date, party_type=pe.party_type, party=pe.party, cost_center=None, 
					save=True, submit=True)
	...
	def make_journal_entry(self, account1, account2, amount, posting_date=None, party_type=None, party=None, cost_center=None, 
		...
		jv.business_unit = self.business_unit
		...
		jv.set("accounts", [
			{
				...
				"party": party if (self.cheque_status == "Cheque Cancelled" or self.cheque_status == "Cheque Rejected") else None,
